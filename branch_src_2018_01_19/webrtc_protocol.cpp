    ...
    else
    {
        ...
        while (BIO_ctrl_pending(bio_in_) > 0)
        {
            ...
            // crc32 test
            {
                ...
                CRC32 crc(CRC32_SCTP);

                uint32_t crc_32 = SctpCRC32(crc_test, ret);
                uint32_t crc_32_myself = crc.GetCrc32(crc_test, ret);
                uint32_t crc_32_webrtc = crc.GetCrc32(crc_test, ret);

                cout << "cal:" << crc_32 << ",in_sctp:" << unused << ",myself:" << crc_32_myself << ",webrtc:" << crc_32_webrtc << endl;
            }
...

                {
                    ...
                    switch (chunk_type)
                    {
                        ...
                        case 1:
                        {
							...
							uint32_t crc_32 = SctpCRC32(bs.GetData(), bs.SizeInBytes());
