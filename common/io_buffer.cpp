...
IoBuffer::~IoBuffer() {
  if (buf_ != NULL) {
    // VERBOSE << LMSG << "capacity_:" << capacity_ << ",Size():" << Size() <<
    // ",CapacityLeft():" << CapacityLeft() << ",buf:" << (void*)buf_ <<
    // std::endl;
...
int IoBuffer::ReadFromFdAndWrite(const int& fd, sockaddr* addr,
                                 socklen_t* addr_len) {
...
int IoBuffer::MakeSpaceIfNeed(const size_t& len) {
  ...
  // std::cout << LMSG << "cur_capacity:" << cur_capacity << ",new_capacity:" <<
...
int IoBuffer::Peek(uint8_t*& data, const size_t& begin_pos, const size_t& len) {
  if (Size() < (begin_pos + len)) {
    std::cout << LMSG << "[" << begin_pos << "," << (begin_pos + len)
              << ") overflow" << std::endl;
    return -1;
  }
