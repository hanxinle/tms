...
inline int IpStr2Num(const std::string& ip_str, uint32_t& ip_num) {
  ...
  if (ret == 1) {
    ...
    std::cout << LMSG << "inet_pton err:" << strerror(errno) << ",ip:" << ip_str
              << std::endl;
    ret = -1;
  }
...
inline int CreateSocketAddrInet(const std::string& ip, const uint16_t& port,
                                sockaddr_in& addr_in) {
...
inline void SocketAddrInetToIpPort(const sockaddr_in& addr_in, std::string& ip,
                                   uint16_t& port) {
...
inline int IpPortToSocketAddr(const std::string& ip, const uint16_t& port,
                              sockaddr_in& addr) {
  int ret = 0;
...
inline int SocketAddrToIpPort(const sockaddr_in& addr, std::string& ip,
                              uint16_t& port) {
...
inline int GetSendBufSize(const int& fd, int& send_buf_size) {
  ...
  int ret =
      getsockopt(fd, SOL_SOCKET, SO_SNDBUF, &send_buf_size, &send_buf_size_len);
...
inline int GetRecvBufSize(const int& fd, int& send_buf_size) {
  ...
  socklen_t send_buf_size_len = sizeof(send_buf_size);
  int ret =
      getsockopt(fd, SOL_SOCKET, SO_RCVBUF, &send_buf_size, &send_buf_size_len);
...
inline int ConnectHost(const int& fd, const std::string& host,
                       const uint16_t& port) {
