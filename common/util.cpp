...
void Util::Daemon() {
  ...
  if (sigaction(SIGHUP, &sa, NULL) < 0) {
    std::cout << LMSG << "sigaction ignore SIGHUP error:" << strerror(errno)
              << std::endl;
    exit(-1);
  }
...
std::string Util::Bin2Hex(const uint8_t* buf, const size_t& len,
                          const size_t& char_per_line, const bool& printf_ascii,
  ...
  for (size_t index = 0; index < len; ++index) {
    ...
    if (printf_ascii) {
      if (isprint((char)buf[index]) && buf[index] != '\r' &&
          buf[index] != '\n') {
...
std::string Util::Bin2Hex(const std::string& str, const size_t& char_per_line,
                          const bool& printf_ascii, const std::string& prefix) {
  return Bin2Hex((const uint8_t*)str.data(), str.length(), char_per_line,
                 printf_ascii, prefix);
}
...
std::string Util::GetNowStr() {
  ...
  size_t ret = strftime(time_printf, sizeof(time_printf), "%Y-%m-%d %H:%M:%S",
                        time_struct);
...
std::string Util::GetNowStrHttpFormat() {
  ...
  size_t ret = strftime(time_printf, sizeof(time_printf), "%a, %d %b %Y %T %z",
                        time_struct);
...
std::string Util::ReadFile(const std::string& file_name) {
  ...
  if (fd < 0) {
    std::cout << LMSG << "open " << file_name << " faild, ret:" << fd
...
std::vector<std::string> Util::SepStr(const std::string& input,
...
void Util::Replace(std::string& input, const std::string& from,
                   const std::string& to) {
  size_t pos = 0;
  size_t next_pos = 0;
