...
if [[ ! -d "${depend_dir}/download/${libsrtp_dir}" && ! -f "${depend_dir}/download/${libsrtp_dir}.tar.gz" ]]; then
    ...
    mkdir ${depend_dir}/download/${libsrtp_dir}
    wget https://github.com/cisco/libsrtp/archive/v2.0.0.tar.gz -O ${depend_dir}/download/${libsrtp_dir}.tar.gz
    tar zxvf ${depend_dir}/download/${libsrtp_dir}.tar.gz -C ${depend_dir}/download/${libsrtp_dir} --strip-components 1
fi
...
if [[ ! -f ${depend_dir}/lib/libsrtp2.a || ! -d ${depend_dir}/include/srtp2 ]]; then
    cd ${depend_dir}/download/${libsrtp_dir}
...
if [[ ! -d "${depend_dir}/download/${libopenssl_dir}" && ! -f "${depend_dir}/download/${libopenssl_dir}.tar.gz" ]]; then
    ...
    mkdir ${depend_dir}/download/${libopenssl_dir}
    wget https://www.openssl.org/source/openssl-1.0.2g.tar.gz -O ${depend_dir}/download/${libopenssl_dir}.tar.gz
    tar zxvf ${depend_dir}/download/${libopenssl_dir}.tar.gz -C ${depend_dir}/download/${libopenssl_dir} --strip-components 1
fi
...
if [[ ! -f ${depend_dir}/lib/libssl.a || ! -d ${depend_dir}/include/openssl ]]; then
    cd ${depend_dir}/download/${libopenssl_dir}
...
if [[ ! -d "${depend_dir}/download/${librapidjson}" ]]; then
    echo "no libsrtp dir and tar file found"
    mkdir ${depend_dir}/download/${librapidjson}
    git clone --depth 1 https://github.com/Tencent/rapidjson.git ${depend_dir}/download/${librapidjson}
fi

if [[ ! -d "${depend_dir}/include/${librapidjson}" ]]; then
    cd ${depend_dir}/download/${librapidjson}
	mkdir build
...
if [[ ! -d "${depend_dir}/download/${libsrt}" ]]; then
    ...
    mkdir ${depend_dir}/download/${libsrt}
    git clone --depth 1 https://github.com/Haivision/srt.git ${depend_dir}/download/${libsrt}
fi

if [[ ! -d "${depend_dir}/include/${libsrt}" ]]; then
    cd ${depend_dir}/download/${libsrt}
	./configure --prefix="${depend_dir}" --enable-profile --disable-c++11 --disable-shared --enable-static && \
	make -j4 && \
	make install
    echo $?
fi
