...
class BitBuffer {
 public:
  ...
  inline bool MoreThanBits(const size_t& bits) { return BitsLeft() >= bits; }
  ...
  inline size_t BytesLeft() { return BitsLeft() / 8; }

  template <typename T>
  int GetBits(const size_t& bits, T& result) {
    if (!MoreThanBits(bits)) {
      std::cout << LMSG << "no more than " << bits
                << " bits, left bits:" << BitsLeft() << std::endl;
      return -1;
    }
    ...
    for (size_t i = 0; i != bits; ++i) {
      ...
      // std::cout << "cur_pos_:" << cur_pos_ << ",mask:" << (uint16_t)mask <<
      // std::endl;
  ...
  size_t HaveReadBytes() { return cur_pos_ / 8; }

  const uint8_t* CurData() const { return data_ + cur_pos_ / 8; }

  size_t CurLen() const { return (bit_len_ - cur_pos_) / 8; }
