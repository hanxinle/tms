...
int HttpParse::Decode(IoBuffer& io_buffer) {
  ...
  int n = next_pos_;
  for (; i < size; ++n, ++i) {
    // std::cout << LMSG << "data[" << i << "]:" << (int)data[i] << "[" <<
    // data[i] << "]" << std::endl;
    if (data[i] == '\r') {
      r_pos_ = n;
    } else if (data[i] == '\n') {
      std::cout << LMSG << "n:" << n << ",i:" << i << ",r_pos_:" << r_pos_
                << ",n_pos_:" << n_pos_ << std::endl;
      if (n == r_pos_ + 1) {
        if (n == n_pos_ + 2)  // \r\n\r\n
        {
        ...
        } else  // \r\n
        {
          ...
          std::cout << LMSG << "http one line [" << key_ << "] => [" << value_
                    << "]" << std::endl;
          ...
          if (key_.find("GET") != std::string::npos) {
            ...
            if (vec.size() >= 2) {
              ...
              if (path_.size() > 0) {
                ...
                std::cout << LMSG << "file_name_:" << file_name_
                          << ",file_type_:" << file_type_ << std::endl;
              }

              // args debug
              for (const auto& kv : args_) {
                std::cout << LMSG << "[" << kv.first << "]=>[" << kv.second
                          << "]" << std::endl;
              }
            }
