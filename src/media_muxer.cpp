...
MediaMuxer::~MediaMuxer() { std::cout << LMSG << std::endl; }
...
void MediaMuxer::PacketTs(const Payload& payload) {
  ...
  ts_queue_[ts_seq_].duration =
      (payload.GetDts() - ts_queue_[ts_seq_].first_dts) / 1000.0;
  ...
  while (i < payload.GetRawLen()) {
    ...
    uint8_t adaptation_field_control =
        1;  // 1:无自适应区 2.只有自适应区 3.同时有负载和自适应区
    ...
    if (i == 0) {
      ...
      //　音频负载通常小于188,这里要做下处理
      if (payload.GetRawLen() + ts_header_size + adaptation_size +
              pes_header_size + extern_data_len <
          188) {
        if (is_video) {
          adaption_stuffing_bytes =
              188 - (payload.GetRawLen() + ts_header_size + adaptation_size +
                     pes_header_size + extern_data_len /*00 00 00 01 09 BC*/);
        } else {
          adaption_stuffing_bytes =
              188 - (payload.GetRawLen() + ts_header_size + adaptation_size +
                     pes_header_size + extern_data_len);
        ...
        // std::cout << LMSG << "payload size:" << payload.GetRawLen() << ",
        // ts_header_size:" << (int)ts_header_size << ", adaptation_size:" <<
        // (int)adaptation_size
        //<< ",pes_header_size:" << (int)pes_header_size << ",extern_data_len:"
        //<< (int)extern_data_len << std::endl;
      ...
      if (left + ts_header_size + adaptation_size <= 188) {
        ...
        if (left + ts_header_size + adaptation_size == 188) {
          ...
          adaptation_field_control = 3;
          adaption_stuffing_bytes =
              188 - (left + ts_header_size + adaptation_size);
          header_size += adaption_stuffing_bytes;
        }
    ...
    ts_bs.WriteBits(
        1, payload_unit_start_indicator);  // payload_unit_start_indicator
    ...
    // VERBOSE << LMSG << "header_size:" << header_size <<
    // ",ts_bs.SizeInBytes():" << ts_bs.SizeInBytes() << ",is_video:" <<
    // is_video << std::endl;
    ...
    if (i == 0 && !is_video && audio_header_.size() >= 2) {
      ...
      adts_header_[3] &= 0xFC;
      adts_header_[3] |=
          (uint8_t)((adts_len & 0x1800) >> 11);  // frame length:value,高2bits
      adts_header_[4] =
          (uint8_t)((adts_len & 0x7f8) >> 3);  // frame length:value,中间8bits
      adts_header_[5] &= 0x1F;
      adts_header_[5] |= ((uint8_t)((adts_len & 0x07) << 5) &
                          0xe0);  // frame length:value,低3bits

      uint16_t calc_len = ((adts_header_[3] & 0x03) << 11) |
                          ((uint16_t)adts_header_[4] << 3) |
                          ((adts_header_[5] & 0xE0) >> 5);
    ...
    ts_queue_[ts_seq_].ts_data.append((const char*)ts_bs.GetData(),
                                      ts_bs.SizeInBytes());
    ...
    if (media_publisher_ != NULL) {
      for (const auto& sub : media_publisher_->GetSubscriber()) {
        if (sub->IsSrt()) {
          sub->SendData(
              std::string((const char*)ts_bs.GetData(), ts_bs.SizeInBytes()));
        }
      }
    }
...
std::string& MediaMuxer::PacketTsPat() {
  ...
  // CRC32从table_id开始(包括table_id),
  // 这里+5是因为上面的table_id用了2个字节,标准是1个字节
...
int MediaMuxer::OnAudio(const Payload& audio_payload) {
  ...
  if (((audio_calc_fps_ != 0 && audio_queue_.size() > 20 * audio_calc_fps_) ||
       audio_queue_.size() >= 800) &&
      ts_queue_.size() > 10) {
...
int MediaMuxer::OnVideo(const Payload& video_payload) {
  ...
  // XXX:可以放到定时器,满了就以后肯定都是满了,不用每次都判断
  if (((video_calc_fps_ != 0 && video_queue_.size() > 20 * video_calc_fps_) ||
       video_queue_.size() >= 800) &&
      ts_queue_.size() > 10) {
    if (video_queue_.begin()->second.IsIFrame()) {
      std::cout << LMSG << "erase " << ts_queue_.begin()->first << ".ts"
                << std::endl;
...
int MediaMuxer::OnVideoHeader(const std::string& video_header) {
  ...
  if (video_header_.empty() && !video_header.empty() &&
      !audio_header_.empty()) {
    pending_arrive = true;
  }
  ...
  if (pending_arrive) {
    if (media_publisher_ != NULL) {
      auto wait_subscriber =
          media_publisher_->GetAndClearWaitHeaderSubscriber();
  ...
  if (p[i] == 0x00 && p[i + 1] == 0x00 && p[i + 2] == 0x00 &&
      p[i + 3] == 0x01) {
    ...
    std::cout << "configuration_version:" << (int)configuration_version
              << std::endl;

    uint8_t avc_profile_indication = 0;
    bit_buffer.GetBytes(1, avc_profile_indication);
    std::cout << "avc_profile_indication:" << (int)avc_profile_indication
              << std::endl;

    uint8_t profile_compatibility = 0;
    bit_buffer.GetBytes(1, profile_compatibility);
    std::cout << "profile_compatibility:" << (int)profile_compatibility
              << std::endl;

    uint8_t avc_level_indication = 0;
    bit_buffer.GetBytes(1, avc_level_indication);
    std::cout << "avc_level_indication:" << (int)avc_level_indication
              << std::endl;
    ...
    std::cout << "length_size_minus_one:" << (int)length_size_minus_one
              << std::endl;
...
int MediaMuxer::OnAudioHeader(const std::string& audio_header) {
  ...
  if (audio_header_.empty() && !audio_header.empty() &&
      !video_header_.empty()) {
    pending_arrive = true;
  }
  ...
  std::cout << LMSG
            << "sampling_frequency_index:" << (uint16_t)sampling_frequency_index
            << std::endl;
  ...
  adts_header_[2] = (audio_object_type - 1)
  ...
  adts_header_[2] |= (channel_config & 0x04) >>
                     2;  // channel configuration:channel_config 高1bit

  adts_header_[3] = (channel_config & 0x03)
...
int MediaMuxer::EveryNSecond(const uint64_t& now_in_ms,
                             const uint32_t& interval, const uint64_t& count) {
  UNUSED(count);

  if (!video_queue_.empty()) {
    std::cout << LMSG << "video queue:" << video_queue_.size() << " ["
              << video_queue_.begin()->first << "-"
              << video_queue_.rbegin()->first << "]" << std::endl;
  }

  if (!audio_queue_.empty()) {
    std::cout << LMSG << "audio queue:" << audio_queue_.size() << " ["
              << audio_queue_.begin()->first << "-"
              << audio_queue_.rbegin()->first << "]" << std::endl;
  }
  ...
  if (pre_calc_fps_ms_ == 0) {
    ...
    std::cout << LMSG << "[STAT] app:" << app_ << ",stream:" << stream_
