...
class RtcpHeader {
 public:
  ...
  inline uint16_t getNackPid() { return ntohs(report.nackPacket.pid); }
  inline void setNackPid(uint16_t pid) { report.nackPacket.pid = htons(pid); }
  inline uint16_t getNackBlp() { return report.nackPacket.blp; }
  inline void setNackBlp(uint16_t blp) { report.nackPacket.blp = blp; }
