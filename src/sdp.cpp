...
int SdpSessionInfo::parse_attribute(const std::string& attribute,
...
int SdpMediaPayload::encode(std::ostringstream& os) {
  os << "a=rtpmap:" << payload_type_ << " " << encoding_name_ << "/"
     << clock_rate_;
  ...
  for (std::set<std::string>::iterator iter = rtcp_fb_.begin();
       iter != rtcp_fb_.end(); ++iter) {
...
SdpMediaDesc::SdpMediaDesc(const std::string& type) { type_ = type; }
...
int SdpMediaDesc::encode(std::ostringstream& os) {
  ...
  os << "m=" << type_ << " ";
  for (std::set<std::string>::iterator iter = protos.begin();
  ...
  for (std::map<int, SdpMediaPayload*>::iterator iter = payloads.begin();
...
int Sdp::encode(std::ostringstream& os) {
  ...
  os << "v=" << version_ << kCRLF;
  os << "o=" << username_ << " " << session_id_ << " " << session_version_
     << " " << nettype_ << " " << addrtype_ << " " << unicast_address_ << kCRLF;
  os << "s=" << session_name_ << kCRLF;
  os << "t=" << start_time_ << " " << end_time_ << kCRLF;

  if (!group_.empty()) {
    os << "a=group:BUNDLE";
    for (std::set<std::string>::iterator iter = group_.begin();
         iter != group_.end(); ++iter) {
      os << " " << *iter;
    }
    os << kCRLF;
  }
  ...
  for (std::vector<SdpMediaDesc*>::iterator iter = media_descs_.begin();
       iter != media_descs_.end(); ++iter) {
...
int Sdp::parse_media_description(const std::string& content) {
  ...
  int fmt;
  while (is >> fmt) {
    cur_media_desc_->payloads.insert(
        std::make_pair(fmt, new SdpMediaPayload(fmt)));
  }
