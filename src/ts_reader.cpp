...
/* stream_id
    ...
    110xxxxx //ISO/IEC 13818-3 or ISO/IEC 11172-3 or ISO/IEC 13818-7 or ISO/IEC
   14496-3 audio stream number x xxxx
    1110xxxx //ITU-T Rec. H.262 | ISO/IEC 13818-2 or ISO/IEC 11172-2 or ISO/IEC
   14496-2 video stream number xxxx
    11110000 //3 ECM_stream
    11110001 //3 EMM_stream
    11110010 //5 ITU-T Rec. H.222.0 | ISO/IEC 13818-1 Annex A or ISO/IEC 13818-
   6_DSMCC_stream
...
int TsReader::ParseTsSegment(const uint8_t* data, const int& len) {
  if (data == NULL || len != kTsSegmentFixedSize ||
      data[0] != kTsHeaderSyncByte) {
    return -1;
  }
...
int TsReader::ParseAdaptation(BitBuffer& bit_buffer) {
  ...
  os << ",elementary_stream_priority_indicator="
  ...
  if (PCR_flag == 0x01) {
    ...
    os << ",pcr="
       << program_clock_reference_base * 300 +
  ...
  if (transport_private_data_flag == 0x01) {
    ...
    os << ",transport_private_data_length="
  ...
  if (adaptation_field_extension_flag == 0x01) {
    ...
    os << ",adaptation_field_extension_length="
...
int TsReader::ParsePMT(BitBuffer& bit_buffer) {
  ...
  while (bit_buffer.BytesLeft() > 4 && es_info_bytes > 0) {
    ...
    os << ",stream_type=" << (int)stream_type
...
int TsReader::ParseDescriptor(BitBuffer& bit_buffer) {
  ...
  std::cout << LMSG << "descriptor_tag=" << (int)descriptor_tag
            << ",descriptor_length=" << (int)descriptor_length << std::endl;

  if (bit_buffer.BytesLeft() < descriptor_length) {
    std::cout << LMSG << "parse failed, bytes_left=" << bit_buffer.BytesLeft()
              << std::endl;
    return -1;
  }
...
int TsReader::ParsePES(std::string& pes) {
  ...
  if (stream_id != kProgramStreamMap && stream_id != kPaddingStream &&
      stream_id != kPrivateStream2 && stream_id != kECMStream &&
      stream_id != kEMMStream && stream_id != kProgramStreamDirector &&
      stream_id != kDSMCCStream && stream_id != kH_222_1_E) {
    uint8_t zero = 0;
  ...
  } else if (stream_id == kProgramStreamMap || stream_id == kPrivateStream2 ||
             stream_id == kECMStream || stream_id == kEMMStream ||
             stream_id == kProgramStreamDirector || stream_id == kDSMCCStream ||
             stream_id == kH_222_1_E) {
    // os << ",pes before stuffing_byte=\n" <<
    // Util::Bin2Hex(bit_buffer.CurData(), bit_buffer.BytesLeft() > 10 ? 10 :
    // bit_buffer.BytesLeft());
...
void TsReader::OnVideo(BitBuffer& bit_buffer, const uint32_t& pts,
                       const uint32_t& dts) {
...
void TsReader::OnH264Video(BitBuffer& bit_buffer, const uint32_t& pts,
                           const uint32_t& dts) {
  ...
  std::cout << "video size=" << bit_buffer.BytesLeft() << ",video frame=\n"
            << Util::Bin2Hex(bit_buffer.CurData(), bit_buffer.BytesLeft() > 256
                                                       ? 256
                                                       : bit_buffer.BytesLeft())
            << std::endl;
  ...
  if (p[i] == 0x00 && p[i + 1] == 0x00 && p[i + 2] == 0x00 &&
    ...
    for (const auto& kv : nals) {
      ...
      if (nal != NULL) {
        ...
        std::cout << LMSG << "nal_ref_idc:" << (int)nal_ref_idc
                  << ", nal_type:" << (int)nal_type << std::endl;
        bool dispatch = true;
        ...
        if (dispatch && frame_callback_) {
          ...
          std::cout << LMSG << "no dispatch nal_ref_idc:" << (int)nal_ref_idc
                    << ", nal_type:" << (int)nal_type << std::endl;
        }
      }
    }
...
void TsReader::OnH265Video(BitBuffer& bit_buffer, const uint32_t& pts,
                           const uint32_t& dts) {
  const uint8_t* p = bit_buffer.CurData();
  uint32_t len = bit_buffer.BytesLeft();
  size_t i = 0;

  std::cout << LMSG << "video size=" << bit_buffer.BytesLeft()
            << ",video frame=\n"
            << Util::Bin2Hex(bit_buffer.CurData(), bit_buffer.BytesLeft() > 256
                                                       ? 256
                                                       : bit_buffer.BytesLeft())
  ...
  if (p[i] == 0x00 && p[i + 1] == 0x00 && p[i + 2] == 0x00 &&
    ...
    for (const auto& kv : nals) {
      ...
      if (nal != NULL) {
        ...
        std::cout << LMSG << "forbidden_zero_bit=" << (int)forbidden_zero_bit
                  << ", nal_type=" << (int)nal_type
                  << ",layer_id=" << (int)layer_id << ",tid=" << (int)tid
                  << std::endl;
        bool dispatch = true;
        ...
        if (nal_type == H265NalType_BLA_N_LP ||
            nal_type == H265NalType_BLA_W_LP ||
            nal_type == H265NalType_BLA_W_RADL ||
            nal_type == H265NalType_CRA_NUT ||
            nal_type == H265NalType_IDR_N_LP ||
            nal_type == H265NalType_IDR_W_RADL) {
          video_frame.SetIFrame();
        } else if (nal_type == H264NalType_SLICE) {
          // XXX:只有解析slice_header, 才能知道SLICE类型, 其他办法都是不准确的
...
void TsReader::OnAudio(BitBuffer& bit_buffer, const uint32_t& pts) {
  ...
  while (bit_buffer.BytesLeft() >= 7) {
    ...
    std::cout << LMSG << "aac_frame_length=" << aac_frame_length
              << ", len=" << len << std::endl;
    if (len > (int)bit_buffer.BytesLeft()) {
      std::cout << LMSG << "no more than " << len << " bytes left, left "
                << bit_buffer.BytesLeft() << std::endl;
      break;
    }
    ...
    audio_frame.SetDts((cal_pts + i * (90000.0 / (44100.0 / 1024.0 / 2))) /
                       90.0);
