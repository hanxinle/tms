...
class WebrtcProtocol : public MediaPublisher,
 ...
 public:
  ...
  void SetLocalUfrag(const std::string& ufrag) { local_ufrag_ = ufrag; }

  void SetLocalPwd(const std::string& pwd) { local_pwd_ = pwd; }

  void SetRemoteUfrag(const std::string& ufrag) { remote_ufrag_ = ufrag; }

  void SetRemotePwd(const std::string& pwd) { remote_pwd_ = pwd; }

  void SubscribeStream();

  void SendVideoData(const uint8_t* data, const int& size,
                     const uint32_t& timestamp, const int& flag);
  void SendAudioData(const uint8_t* data, const int& size,
                     const uint32_t& timestamp, const int& flag);

  int ProtectRtp(const uint8_t* un_protect_rtp, const int& un_protect_rtp_len,
                 uint8_t* protect_rtp, int& protect_rtp_len);
  int UnProtectRtp(const uint8_t* protect_rtp, const int& protect_rtp_len,
                   uint8_t* un_protect_rtp, int& un_protect_rtp_len);

  int ProtectRtcp(const uint8_t* un_protect_rtp, const int& un_protect_rtp_len,
                  uint8_t* protect_rtp, int& protect_rtp_len);
  int UnProtectRtcp(const uint8_t* protect_rtp, const int& protect_rtp_len,
                    uint8_t* un_protect_rtp, int& un_protect_rtp_len);

  bool DtlsHandshakeDone() { return dtls_handshake_done_; }

  UdpSocket* GetUdpSocket() { return (UdpSocket*)socket_; }
